...
function stop {
  ...
  docker-compose --project-name weather_analysis down
}

function cleanup {
  echo "Removing volume"
  docker volume rm weather_analysis_postgres-data
  docker volume rm weather_analysis_superset
  docker volume rm weather_analysis_postgres-airflow-data
}

function start {
  echo "Starting up"
  docker-compose --project-name weather_analysis up -d
}
...
function update {
  ...
  docker-compose --project-name weather_analysis pull
...
case $1 in
  ...
  logs )
  docker-compose --project-name weather_analysis logs -f
    ;;
